...
- Global Admin Console中可用的日志仅包括全局管理员有权访问的事件。 它们不包括用户分配或用户事件。 [了解更多](https://helpx.adobe.com/enterprise/using/admin-console.html)有关每个控制台提供的不同功能。
...
审核日志会显示已过滤事件的以下信息：
   ...
   | IP地址 | 执行操作的计算机的IP地址。 通常反映物理位置，但可以是代理服务器或VPN地址。 |
   | 组织 | 受事件影响的组织的名称。 |
...
1. 您可以使用以下选项筛选审核日志：
   ...
   - 定义日期范围。
   - 按事件名称筛选。
   - 您可以合并过滤器以缩小结果范围，例如查看特定组织过去七天的事件。
...
1. 登录到[Global Admin Console](https://global-admin-console.adobe.com/insights)，然后导航到&#x200B;**[!UICONTROL 分析]** > **[!UICONTROL 导出报表]**。
   ...
   此时会显示过去90天内生成的报告。 90天完成后，您可以再次生成报表。 了解如何生成[组织结构](https://helpx.adobe.com/enterprise/global-admin-console/export-and-import-data.html#export-and-import-organization-structure)的报告。
